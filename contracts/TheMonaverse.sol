...
abstract contract mTokenInterface {
...
abstract contract ControllerInterface {
    ...
    function getXVSAddress() external view virtual returns (address);
}
...
contract TheMonaverse is Initializable {
    ...
    mapping(address => uint) mTokenBalances;

    event Deposit(address owner, uint amount, uint mTokenChange);
    event Redeemed(address owner, uint amount, uint redeemFee, uint amountWithoutFee, uint mTokenChange);
    ...
    function initialize(address _mToken, address _governance) initializer public {
        require(_mToken != address(0), "_mToken is zero address"); 
        require(_governance != address(0), "_governance is zero address");

        mToken = _mToken;
        underlyingAsset = mTokenInterface(mToken).underlying();
        governance = _governance;
        redeemFeeConfig[day7Seconds] = 5;
        redeemFeeConfig[day14Seconds] = 4;
        redeemFeeConfig[day30Seconds] = 3;
        redeemFeeConfig[day90Seconds] = 2;
        redeemFeeConfig[day180Seconds] = 1;
    }
    ...
    function deposit(uint depositAmount) nonReentrant mine external {
        ...
        uint mTokenAmountBeforeDeposit = IERC20(mToken).balanceOf(address(this));
        IERC20(underlyingAsset).approve(mToken, depositAmount);
        uint error = mTokenInterface(mToken).mint(depositAmount);
        require(error == 0, "Error to mint mToken");
        uint mTokenAmountAfterDeposit = IERC20(mToken).balanceOf(address(this));
        uint mTokenAmountChange = mTokenAmountAfterDeposit.sub(mTokenAmountBeforeDeposit);

        mTokenBalances[msg.sender] = mTokenBalances[msg.sender].add(mTokenAmountChange);

        emit Deposit(msg.sender, depositAmount, mTokenAmountChange);
    }

    function redeem(uint redeemAmount) nonReentrant mine external {
        require(mTokenBalances[msg.sender] > 0, "Balance should > 0");
        require(redeemAmount > 0, "Redeem amount should > 0");

        uint mTokenAmountBeforeRedeem = IERC20(mToken).balanceOf(address(this));
        uint error = mTokenInterface(mToken).redeemUnderlying(redeemAmount);
        require(error == 0, "Error to redeem");
        uint mTokenAmountAfterRedeem = IERC20(mToken).balanceOf(address(this));
        uint mTokenAmountChange = mTokenAmountBeforeRedeem.sub(mTokenAmountAfterRedeem);

        require(mTokenBalances[msg.sender] >= mTokenAmountChange, "mTokenBalance is not enough to redeem");
        mTokenBalances[msg.sender] = mTokenBalances[msg.sender].sub(mTokenAmountChange);
        ...
        emit Redeemed(msg.sender, redeemAmount, redeemFee, actualRedeem, mTokenAmountChange);
    }
    ...
    function calculateFee(address saver, uint redeemAmount) public view returns(uint) {
        ...
        return redeemAmount.mul(feeRate).div(1000);
    }
    ...
    function redeemAll() nonReentrant external {
        ...
        uint mTokenBalance = mTokenBalances[msg.sender];
        mTokenBalances[msg.sender] = 0;
        uint error = mTokenInterface(mToken).redeem(mTokenBalance);
        require(error == 0, "Error to redeem");
        ...
        emit Redeemed(msg.sender, tokenBalance, redeemFee, actualRedeem, mTokenBalance);
    }
    ...
    function claimXVS(address receiver) public {
        if (receiver != governance) {
            require(msg.sender == governance, "Not governance");
        }

        address comtrollerAddr = mTokenInterface(mToken).comptroller();
        ControllerInterface comtroller = ControllerInterface(comtrollerAddr);
        comtroller.claimVenus(address(this));

        address xvsAddr = comtroller.getXVSAddress();
        uint xvsAmount = IERC20(xvsAddr).balanceOf(address(this));
        IERC20(xvsAddr).transfer(receiver, xvsAmount);
    }

    function claimXVS() public {
        claimXVS(governance);
    }

    function mTokenBalanceOf(address owner) external view returns(uint) {
        return mTokenBalances[owner];
    }

    function tokenBalanceOf(address owner) public returns(uint) {
        uint exchangeRateCurrent = mTokenInterface(mToken).exchangeRateCurrent();
        uint balance = mTokenBalances[owner].mul(exchangeRateCurrent).div(1e18);
        return balance;
    }

    function storedTokenBalanceOf(address owner) view external returns(uint) {
        uint exchangeRateCurrent = mTokenInterface(mToken).exchangeRateStored();
        uint balance = mTokenBalances[owner].mul(exchangeRateCurrent).div(1e18);
        return balance;
    }
    ...
    function updateMiningIndex() internal {
        ...
        if (deltaBlocks > 0 && miningSpeed > 0 && miningBlock > 0) {
            uint totalmTokenBalance = mTokenInterface(mToken).balanceOf(address(this));
            uint miningAccrued = deltaBlocks.mul(miningSpeed);
            uint ratio = totalmTokenBalance > 0 ? miningAccrued.mul(1e18).div(totalmTokenBalance) : 0;
    ...
    function distributeMining() internal {
        ...
        if (currentSaverMiningIndex > 0) {
            ...
            uint savermTokens = mTokenBalances[msg.sender];
            uint saverDelta = savermTokens.mul(deltaIndex).div(1e18);
